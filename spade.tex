...
\begin{subequations}
  \begin{align}
    ...
    b(x,t) &= \left(\alpha_1 x + \alpha_2 x^2\right)\label{seq:2c},%\exp\left(\psi \cos(t-\phi)\right)/ 2\pi I_0(\psi)
  \end{align}
\end{subequations}
...
The model is calibrated against fishery catch-at-size data. This comes from two sources: commercial catch data (a scalar time series representing removals from the population), and scientific monitoring program data on the size structure of the commercial catch. These are combined to produce the `observation': $c(x,t)$, the catch rate (in kilograms per centimetre) at time $t$. It is this observation to which the model is fit:
\begin{equation}
H(\theta) = \int_0^T \int_0^\omega |s(x)w(x)f(t;\theta)u(x,t;\theta) - c(x,t)| \,dx\,dt
\end{equation}
Optimisation is a hard problem in general, and optimisation routines that make use of the gradient of the objective function (with respect to the model parameters) are usually significantly more efficient. The gradient is given by
\begin{equation}
%\frac{d H(\theta)}{d\theta}=
\int_0^T \int_0^\omega
\begin{cases}
-s(x)w(x)f_\theta(t;\theta)u(x,t;\theta) - s(x)w(x)f(t;\theta)p(x,t;\theta), &  s(x)w(x)f(t;\theta)u(x,t;\theta) < c(x,t)\\
s(x)w(x)f_\theta(t;\theta)u(x,t;\theta) + s(x)w(x)f(t;\theta)p(x,t;\theta), & s(x)w(x)f(t;\theta)u(x,t;\theta)>c(x,t) 
\end{cases}\,dx\,dt
\end{equation}
and where $p(x,t;\theta)=u_\theta(x,t;\theta)$ is obtained by solving the sensitivity-PDE \citep{Borggaard1997,Li2004}. %,Stanley2002}.
\begin{equation}
