...
\begin{equation}
  \begin{split}    
    \frac{d H_1(\theta)}{d\theta}&=G_1(\theta)=\int_0^T 2\left(\int_0^\omega s(x)w(x)f(t;\theta)u(x,t;\theta)\,dx - c(t)\right)\\
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \State $X_{j+1}^{n+1/8} \gets X_j^n + (k/8) g(X_j^n)$
    \State $U_{j+1}^{n+1/8} \gets U_j^n \exp\left(-(k/8) z^*(X_j^n,\mathcal{Q}(\mathbf{X}^n,\mathbf{U}^n))\right)$
    \State $X_{j+1}^{n+1/4} \gets X_j^n + (k/4) g(X_j^n)$
    \State $U_{j+1}^{n+1/4} \gets U_j^n \exp\left(-(k/4) z^*(X_j^n,\mathcal{Q}(\mathbf{X}^n,\mathbf{U}^n))\right)$
    \State $P_{j+1}^{n+1/4} \gets P_j^n \exp\left(-(k/4)z^*(X_{j}^{n},\mathcal{Q}(\mathbf{X}^{n},\mathbf{U}^{n}),t^{n})\right)-\exp\left(-(k/4)z^*(X_{j}^{n},\mathcal{Q}(\mathbf{X}^{n},\mathbf{U}^{n}),t^{n})\right)\times$\\ \hspace{1cm} $(k/4)\left(s(X_j^n)e(t^n)+\gamma\mathcal{Q}(\mathbf{X}^n,\mathbf{P}^n)\right)U_j^n$
    \State $X_{j+1}^{n+3/8} \gets X_j^n + (3k/8) g(X_j^n)$
    \State $U_{j+1}^{n+3/8} \gets U_j^n \exp\left(-(3k/8) z^*(X_j^n,\mathcal{Q}(\mathbf{X}^n,\mathbf{U}^n))\right)$
    \State $X_{j+1}^{n+1/2} \gets X_j^n + (k/2) g(X_j^n)$
    \State $U_{j+1}^{n+1/2} \gets U_j^n \exp\left(-(k/2) z^*(X_j^n,\mathcal{Q}(\mathbf{X}^n,\mathbf{U}^n))\right)$
    \State $P_{j+1}^{n+1/2} \gets P_j^n \exp\left(-(k/2)z^*(X_{j+1}^{n+1/4},\mathcal{Q}(\mathbf{X}^{n+1/4},\mathbf{U}^{n+1/4}),t^{n+1/4})\right)-$\\ \hspace{1cm} $\exp\left(-(k/2)z^*(X_{j+1}^{n+1/4},\mathcal{Q}(\mathbf{X}^{n+1/4},\mathbf{U}^{n+1/4}),t^{n+1/4})\right)\times$\\ \hspace{1cm} $(k/2)\left(s(X_{j+1}^{n+1/4})e(t^{n+1/4})+\gamma\mathcal{Q}(\mathbf{X}^{n+1/4},\mathbf{P}^{n+1/4})\right)U_{j+1}^{n+1/4}$\\ \hspace{1cm} $ \exp\left(-(k/4)z^*(X_{j+1}^{n+1/8},\mathcal{Q}(\mathbf{X}^{n+1/8},\mathbf{U}^{n+1/8}),t^{n+1/8})\right)$
    \State $X_{j+1}^{n+6/8} \gets X_j^n + (6k/8) g(X_j^n)$
    \State $U_{j+1}^{n+6/8} \gets U_j^n \exp\left(-(6k/8) z^*(X_j^n,\mathcal{Q}(\mathbf{X}^n,\mathbf{U}^n))\right)$
    \State $P_{j+1}^{n+1} \gets P_j^n \exp\left(-k z^*(X_{j}^{n+1/2},\mathcal{Q}(\mathbf{X}^{n+1/2},\mathbf{U}^{n+1/2}),t^{n+1/2})\right)-$\\ \hspace{1cm}$\exp\left(-k z^*(X_{j+1}^{n+1/2},\mathcal{Q}(\mathbf{X}^{n+1/2},\mathbf{U}^{n+1/2}),t^{n+1/2})\right)\times$\\ \hspace{1cm}$\left[(k/2)\left(s(X_{j+1}^{n+1/4})e(t^{n+1/4})+\gamma \mathcal{Q}(\mathbf{X}^{n+1/4},\mathbf{P}^{n+1/4})\right)U_{j+1}^{n+1/4}\times\right.$\\ \hspace{1cm}$\left.\exp\left((k/4)z^*(X_{j+1}^{n+1/8},\mathcal{Q}(\mathbf{X}^{n+1/8},\mathbf{U}^{n+1/8}),t^{n+1/8})\right)\right.\times$\\ \hspace{1cm}$\left.(k/2)\left(s(X_{j+1}^{n+3/4})e(t^{n+3/4})+\gamma \mathcal{Q}(\mathbf{X}^{n+3/4},\mathbf{P}^{n+3/4})\right)U_{j+1}^{n+3/4}\times\right.$\\ \hspace{1cm}$\left.\exp\left(-(3k/4) z^*(X_{j+1}^{n+3/8},\mathcal{Q}(\mathbf{X}^{n+3/8},\mathbf{U}^{n+3/8}),t^{n+3/8})\right)\right]$
  \end{algorithmic}
\end{algorithm}

% \State $X_0^{n+1} \gets 0$

% + k z^*(X_{j}^{n+3/2},\mathcal{Q}(\mathbf{X}^{n+3/2},\mathbf{U}^{n+3/2}),t^{n+3/2})\right),$\\ \hspace{1cm}$0 \leq j \leq J$
...
\begin{equation}
  \nabla H_*(\tilde{\theta};\iota) \equiv \langle \xi_2 \frac{d H_2}{d \alpha_1}, \xi_2 \frac{d H_2}{d \alpha_2}, \xi_2 \frac{d H_2}{d \beta}, \xi_2 \frac{d H_2}{d \gamma}, \xi_2 \frac{d H_2}{d \kappa} + \xi_3 \frac{d H_3}{d \kappa} + \xi_4 \frac{d H_4}{d \kappa}, \xi_2 \frac{d H_2}{d \omega} + \xi_3 \frac{d H_3}{d \omega} + \xi_4 \frac{d H_4}{d \omega}\rangle
\end{equation}
