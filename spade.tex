...
g(0)u(0,t) &=  \int_0^{\omega} b(x,t) u(x,t)\,dx\label{eq:1.2}\\ 
...
\begin{subequations}
  \begin{align}
    ...
    b(x,t) &= \left(\alpha_1 x + \alpha_2 x^2\right) \exp\left(\psi \cos(t-\phi)\right)/ 2\pi I_0(\psi)    \label{seq:2c},
  \end{align}
\end{subequations}
...
Let the observed catch rate at time $t$ be $c(t)$, let the observed effort rate at time $t$ be $e(t)$, let the observed size-structure at time $t_j$ be represented by the function $l_j(x)$, and let $\theta=\{\alpha_1,\alpha_2,\beta,\gamma,\kappa,\omega,\iota\}$. Then we have an objective function for catch
\begin{equation}
...
\begin{equation}
  H_2(\theta) = \sum_j \int_0^\omega l_j'(x)-s(x)f(t;\theta)u(x,t;\theta)\,dx
\end{equation}
where 
\begin{equation}
   l_j'(x) = \frac{c(t)}{\int_0^\omega l_j(x)w(x)\,dx}l_j(x)% \frac{s(x)u(x,t_j;\theta)}{\int s(x)u(x,t_j;\theta)\,dx },
\end{equation}
...
and 
\begin{equation}
  \frac{d H_2(\theta)}{d\theta} = \sum_j \int_0^\omega l_j(x)\frac{u(x,t_j;\theta)\int_0^\omega s(x)p(x,t_j;\theta)\,dx - p(x,t_j;\theta) \int_0^\omega s(x)u(x,t_j;\theta)\,dx }{u(x,t_j;\theta) \int_0^\omega s(x)u(x,t_j;\theta)\,dx}\,dx
\end{equation}
...
\begin{algorithm}
  \caption{Sensitivity PDE numerical solution}\label{alg:sens}
  \begin{algorithmic}
...
\begin{subequations}
  \begin{align*}
    \frac{dU}{dt} &= -(\beta+\gamma U) U + \left(\alpha_1 V + \alpha_2 W\right)\frac{e^{\psi \cos(\theta-\phi)}}{2\pi I_0(\psi)}\\
