...
void grad_alpha2_fast(void *args)
{
  ...
  */
  ...
  Real Za2 = (6*kk*kk*ww*ww*(zeta+9*kk*ww*(a1 + 2*a2*ww)) / (zeta*pow(9*a1*kk*kk*ww + 18*a2*kk*kk*ww*ww + kk*zeta,2./3.)) ) * ( (1/(pow(2.,1./3.)*pow(3.,2./3.))) - ( pow(2./3,1./3.)*kk*(kk+a1*ww)) / (pow(9*a1*kk*kk*ww + 18*a2*kk*kk*ww*ww + kk*zeta,2./3.)) );

  for (int j=0;j<=d.J;j++)
    {
      p[j] = pow(1-x[j]/ww,Z/kk - 2.) * ( (Z - bb - kk)/(gg*Z) * ( 2*kk*ww/(Z+kk) - (2*a2*kk*ww/pow(Z+kk,2.)) * Za2) + (Za2/(gg*Z))*(a1 + 2*a2*kk*ww/(Z+kk))*((bb+kk)/Z + log(1 - x[j]/ww) * (Z-bb-kk)/kk) );
  ...
  for (int i=1;i<=d.I;i++)
  {
	...
	ph[j] = p[j]*exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * _e(d.eff,t) - kk)) - exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * _e(d.eff,t) - kk))*(k/2)*gg*P*u[j];
      ...
      // reproduce ph
      Real ph_0 = a1*xh[0]*ph[0]*xh[0] + a2*xh[0]*xh[0]*ph[0]*xh[0] + xh[0]*uh[0]*xh[0]*xh[0];

      for (int j=0;j<d.J;j++)
	ph_0 += ( a1*xh[j]*ph[j] + a2*xh[j]*xh[j]*ph[j] + xh[j]*xh[j]*uh[j] + a1*xh[j+1]*ph[j+1] + a2*xh[j+1]*xh[j+1]*ph[j+1] + xh[j+1]*xh[j+1]*uh[j+1]) * (xh[j+1]-xh[j]);
	...
	un[0] += (_b(a1,a2,xn[j])*un[j] + _b(a1,a2,xn[j+1])*un[j+1]) * (xn[j+1]-xn[j]);
      ...
      // reproduce p
      pn[0] = a1*xn[1]*pn[1]*xn[1] + a2*xn[1]*xn[1]*pn[1]*x[1] + xn[1]*xn[1]*un[1]*xn[1];

      for (int j=1;j<d.J;j++)
	pn[0] += ( a1*xn[j]*pn[j] + a2*xn[j]*xn[j]*pn[j] + xn[j]*xn[j]*un[j] + a1*xn[j+1]*pn[j+1] + a2*xn[j+1]*xn[j+1]*pn[j+1] + xn[j+1]*xn[j+1]*un[j+1]) * (xn[j+1]-xn[j]);
  ...
  */
  ...
  Real * restrict ph2 = (Real *) calloc(d.J,sizeof(Real));  
	...
	u[0] += (_b(a1,a2,x[j])*u[j] + _b(a1,a2,x[j+1])*u[j+1]) * (x[j+1]-x[j]);
      ...
      // reproduce p
      p[0] = a1*x[1]*p[1]*x[1] + a2*x[1]*x[1]*p[1]*x[1] + x[1]*x[1]*u[1]*x[1];

      for (int j=1;j<d.J;j++)
	p[0] += ( a1*x[j]*p[j] + a2*x[j]*x[j]*p[j] + x[j]*x[j]*u[j] + a1*x[j+1]*p[j+1] + a2*x[j+1]*x[j+1]*p[j+1] + x[j+1]*x[j+1]*u[j+1]) * (x[j+1]-x[j]);
