...
void grad_omega_clean(void* args)
{
  ...
  */
  ...
  Real Zw =  (3*kk*kk*(aa*A1*zeta+4*aa*A2*ww*zeta-2*aa*A1*pow(kk+aa*A1*ww,2.)+18*aa*A2*kk*ww*ww*(aa*A1+2*aa*A2*ww)+9*kk*ww*pow(aa*A1+2*aa*A2*ww,2.) ) / (zeta*pow(9*aa*A1*kk*kk*ww+18*aa*A2*kk*kk*ww*ww+kk*zeta,2./3)) ) * ( 1./(pow(2.,1/3.)*pow(3.,2/3.)) - pow(2/3.,1/3.)*kk*(kk+aa*A1*ww) / pow(9*aa*A1*kk*kk*ww+18*aa*A2*kk*kk*ww*ww+kk*zeta,2./3) ) + pow(2/3.,1/3.)*aa*A1*kk / pow(9*aa*A1*kk*kk*ww+18*aa*A2*kk*kk*ww*ww+kk*zeta,1./3) ;

  for (int j=0;j<core_x->n;j++)
    p[j] = pow(1-x[j]/ww,Z/kk - 2.) * ( (Z-bb-kk)/(gg*Z) *( 2*kk*ww/(Z+kk) - (2*aa*A2*kk*ww/pow(Z+kk,2.))*Zw + (aa*A1 + 2*aa*A2*kk*ww/(Z+kk))*x[j]*(Z-2*kk)/(kk*ww*(ww-x[j]))) + (Zw/(gg*Z)) * (aa*A1+2*aa*A2*kk*ww/(Z+kk))*( (bb+kk)/Z + log(1-x[j]/ww) * (Z-bb-kk)/kk ) );
  ...
  for (int i=1;i<core_x->m;i++)
	...
	Uh += .5 * (xh[j+1] - xh[j]) * (uh[j+1] + uh[j]);
      ...
      ph[0] = p[0]*tmp - tmp * (k/2)*( gg*P*u[0] + kk * (u[1]-u[0])/(x[1]-x[0]) );
      
      for (int j=1;j<core_x->n-1;j++)
	{

	  Real tmp = exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * e(eff,k,t,d->Y) - kk));
	  ph[j] = p[j]*tmp - tmp * (k/2)*( gg*P*u[j] + kk * .5*((u[j+1]-u[j]+1e-15)/(x[j+1]-x[j]+1e-15) + (u[j]-u[j-1]+1e-15)/(x[j]-x[j-1]+1e-15)) ) ;

	}					   

      int T=core_x->n-1;
      tmp = exp(-(k/2)*(bb + gg*U + s(x[T]) * ii * e(eff,k,t,d->Y) - kk));
      ph[T] = p[T]*tmp - tmp * (k/2)*( gg*P*u[T] );        
      // reproduce ph
	...
	ph_0 += (b(aa,xh[j-1])*ph[j-1] + b(aa,xh[j])*ph[j])*(xh[j]-xh[j-1]);
      ...
      ph_0 = (ph_0 - 2*kk*uh_0) / (2*kk*ww - xh[0]*b(aa,0));
      ...
      for (int j=core_x->n-1;j>1;j--)
	{
	  ...
	  p[j] = p[j-1]*tmp1 - tmp1 * k*( gg*Ph*uh[j-1] + kk * .5 * ((uh[j-1]-uh[j-2]+1e-15)/(xh[j-1]-xh[j-2]+1e-15) + (uh[j]-uh[j-1]+1e-15)/(xh[j]-xh[j-1]+1e-15)) ) * tmp2;
      ...
      Real bbb = k*( gg*Ph*uh[j-1] + kk * .5 * ((uh[j-1]-uh_0)/xh[j-1] + (uh[j]-uh[j-1])/(xh[j]-xh[j-1]) ));
      Real e1 = p[j-1];
	...
	p[0] += (b(aa,x[j])*p[j] + b(aa,x[j+1])*p[j+1])*(x[j+1]-x[j]);
      ...
      p[0] = (p[0] - 2*kk*u[0]) / (2*kk*ww - x[1]*b(aa,0));
