...
//  VEC *grad = (*grad_args).g;
  ...
  for (int i=1;i<x->m;i++)
    { 
      ...
      for (int j=1;j<=x->n;j++)
	ph->ve[j] = pt->ve[j-1]*exp(-(k/2)*zstar(eff,bb,gg,kk,ii,t,xt->ve[j-1],Ui->ve[i-1],k,d->Y)) - exp(-(k/2)*zstar(eff,bb,gg,kk,ii,t,xt->ve[j-1],Ui->ve[i-1],k,d->Y))*(k/2)*(1+gg*Pi->ve[i-1])*ut->ve[j-1];
