...
void grad_iota(void* args)
{
  ...
  for (int i=1;i<x->m;i++)
    { 
      ...
      for (int j=1;j<=x->n;j++)
	ph->ve[j] = pt->ve[j-1]*exp(-(k/2)*zstar(eff,bb,gg,kk,ii,t,xt->ve[j-1],Ui->ve[i-1],k,d->Y)) - exp(-(k/2)*zstar(eff,bb,gg,kk,ii,t,xt->ve[j-1],Ui->ve[i-1],k,d->Y))*(k/2)*(1e-3*s(xt->ve[j-1])*e(eff,k,t,d->Y)+gg*Pi->ve[i-1])*ut->ve[j-1];
      ...
      for (int j=1;j<=x->n;j++)
	{
	  Real b = k*(1e-3*s(xht->ve[j])*e(eff,k,th,d->Y)+gg*Ph)*uht->ve[j];
