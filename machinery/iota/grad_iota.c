  ...
  */
  ...
  for (int j=0;j<=d.J;j++)
    {
      ...
      l[j] = 1e-3*w(x[j])*s(x[j])*u[j]*1e-3*_e(d.eff,0) + 1e-3*w(x[j])*s(x[j])*p[j]*ii*_e(d.eff,0);
    }
  ...
  for (int i=1;i<=d.I;i++)
  {
	...
	Uh += .5 * (xh[j+1] - xh[j]) * (uh[j+1] + uh[j]);
      ...
      // calculate ph
      for (int j=0;j<=d.J;j++)
	ph[j] = p[j]*exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * _e(d.eff,t) - kk)) - exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * _e(d.eff,t) - kk))*(k/2)*(1e-3*s(x[j])*_e(d.eff,t)+gg*P)*u[j];
      ...
      for (int j=d.J+1;j>0;j--)
	{
	  ...
	  pn[j] = p[j-1]*tmp1 - tmp1 * k*(1e-3*s(xh[j])*_e(d.eff,th)+gg*Ph)*uh[j-1] * tmp2;
	}
      ...
      for (int j=0;j<=d.J+1;j++)
	{
	  ...
	  l[j] = 1e-3*w(xn[j])*s(xn[j])*un[j]*1e-3*_e(d.eff,k*i) + 1e-3*w(xn[j])*s(xn[j])*pn[j]*ii*_e(d.eff,k*i);
	}
  ...
  */
  ...
  Real * restrict ph2 = (Real *) calloc(d.J,sizeof(Real));  
      ...
      for (int j=d.J;j>0;j--)
	{
	  ...
	  Real tmp1 = exp(-k*(bb+gg*Uh+s(xh[j-1])*ii*_e(d.eff,th)-kk));
	  Real tmp2 = exp((k/2)*(bb+gg*Uh+s(xh[j-1])*ii*_e(d.eff,th)-kk));
	  p[j] = p[j-1]*tmp1 - tmp1 * k*(1e-3*s(xh[j])*_e(d.eff,th)+gg*Ph)*uh[j-1] * tmp2;
	}
      ...
      for (int j=0;j<=d.J;j++)
	{
	  ...
	  l[j] = 1e-3*w(x[j])*s(x[j])*u[j]*1e-3*_e(d.eff,k*i) + 1e-3*w(x[j])*s(x[j])*p[j]*ii*_e(d.eff,k*i);
	}
