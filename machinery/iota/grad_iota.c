...
void grad_iota(void *args)
{
  ...
  */
  ...
  for (int j=0;j<=d.J;j++)
    {
      ...
      l[j] = w(x[j])*s(x[j])*u[j]*1e-3*_e(d.eff,d.k,d.k*(0-d.N)) + w(x[j])*s(x[j])*p[j]*ii*_e(d.eff,d.k,d.k*(0-d.N));
    }
  ...
  for (int j=0;j<=d.J;j++)
    o[j] = 2*(r[j] - _c(d.cat,d.k,d.k*(0-d.N)) * (d.p[0][j] + (1-d.Qp[0]) * r[j]/C))*(l[j] - _c(d.cat,d.k,d.k*(0-d.N))*(1 - d.Qp[0]) * ( l[j] * C - r[j] * L ) / (C*C)); 

  for (int j=0;j<=d.J;j++)
    m[j] = pow(_c(d.cat,d.k,d.k*(0-d.N)) * (d.p[0][j] + (1-d.Qp[0]) * r[j]/C) - r[j],2.0);                      
  ...
  for (int i=1;i<=d.I;i++)
  {
	...
	Uh += .5 * (xh[j+1] - xh[j]) * (uh[j+1] + uh[j]);
      ...
      // calculate ph
      for (int j=0;j<=d.J;j++)
	ph[j] = p[j]*exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * _e(d.eff,k,t) - kk)) - exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * _e(d.eff,k,t) - kk))*(k/2)*(1e-3*s(x[j])*_e(d.eff,k,t)+gg*P)*u[j];
  ...
  Real * restrict ph2 = (Real *) calloc(d.J,sizeof(Real));  
      ...
      for (int j=d.J;j>0;j--)
	{
	  ...
	  p[j] = p[j-1]*tmp1 - tmp1 * k*(1e-3*s(xh[j])*_e(d.eff,k,th)+gg*Ph)*uh[j-1] * tmp2;
	}
      ...
      for (int j=0;j<=d.J;j++)
	{
	  ...
	  l[j] = w(x[j])*s(x[j])*u[j]*1e-3*_e(d.eff,d.k,d.k*(i-d.N)) + w(x[j])*s(x[j])*p[j]*ii*_e(d.eff,d.k,d.k*(i-d.N));
	}
  ...
  */
  ...
  for (int i=1;i<core_x->m;i++)
	...
	Uh += .5 * (xh[j+1] - xh[j]) * (uh[j+1] + uh[j]);
      ...
      // calculate ph
      for (int j=0;j<core_x->n;j++)
	ph[j] = p[j]*exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * e(eff,k,t,d->Y) - kk)) - exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * e(eff,k,t,d->Y) - kk))*(k/2)*(1e-3*s(x[j])*e(eff,k,t,d->Y) + gg*P)*u[j];
      ...
      for (int j=core_x->n-1;j>0;j--)
	{
	  ...
	  p[j] = p[j-1]*tmp1 - tmp1 * k*(1e-3*s(x[j])*e(eff,k,t,d->Y) +gg*Ph)*uh[j-1] * tmp2;
	}
...
void grad_iota(void* args) {
  ...
  for (int i=1;i<x->m;i++)
	...
	ph->ve[j+1] = pt->ve[j]*exp(-(k/2)*zstar(eff,bb,gg,kk,ii,t,xt->ve[j],Ui->ve[i-1],k,d->Y)) - exp(-(k/2)*zstar(eff,bb,gg,kk,ii,t,xt->ve[j],Ui->ve[i-1],k,d->Y))*(k/2)*(1e-3*s(xt->ve[j])*e(eff,k,t,d->Y)+gg*Pi->ve[i-1])*ut->ve[j];
	...
	for (int j=0;j<=terminator;j++)
	  {
	    Real b = k*(1e-3*s(xht->ve[j+1])*e(eff,k,th,d->Y)+gg*Ph)*uht->ve[j+1];
  ...
  */
  ...
  parameters->iota.gradient = G(p, x, u, d, parameters->iota.value);
