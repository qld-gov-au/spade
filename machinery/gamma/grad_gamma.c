...
void grad_gamma(void *args)
{
  ...
  */
  ...
  for (int j=0;j<=d.J;j++)
    {
      p[j] = -pow(1-x[j]/ww,Z/kk - 2.) * ((Z-bb-kk)/(1e7*gg*gg*Z)) * (a1 + 2*a2*kk*ww/(Z+kk));

      r[j] = w(x[j])*s(x[j])*u[j]*ii*_e(d.eff,d.k,d.k*(0-d.N));
      l[j] = w(x[j])*s(x[j])*p[j]*ii*_e(d.eff,d.k,d.k*(0-d.N));
    }
  ...
  for (int i=1;i<=d.I;i++)
  {
	...
	Uh += .5 * (xh[j+1] - xh[j]) * (uh[j+1] + uh[j]);
      ...
      // calculate ph
      for (int j=0;j<=d.J;j++)
	ph[j] = p[j]*exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * _e(d.eff,k,t) - kk)) - exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * _e(d.eff,k,t) - kk))*(k/2)*(1e-7*U+gg*P)*u[j];
  ...
  Real * restrict ph2 = (Real *) calloc(d.J,sizeof(Real));  
      ...
      for (int j=d.J;j>0;j--)
	{
	  ...
	  p[j] = p[j-1]*tmp1 - tmp1 * k*(1e-7*Uh+gg*Ph)*uh[j-1] * tmp2;
	}
      ...
      for (int j=0;j<=d.J;j++)
	{
	  ...
	  l[j] = w(x[j])*s(x[j])*p[j]*ii*_e(d.eff,d.k,d.k*(i-d.N));
	}
...
void grad_gamma_clean(void* args)
{
  ...
  */
  ...
  for (int j=0;j<core_x->n;j++)
    p[j] = -pow(1-x[j]/ww,Z/kk - 2.) * ( (Z-bb-kk)/(1e7*gg*gg*Z)) * (aa*A1 + 2*aa*A2*kk*ww/(Z+kk));  // 1e7 should explain (rescaling)
  ...
  for (int i=1;i<core_x->m;i++)
	...
	Uh += .5 * (xh[j+1] - xh[j]) * (uh[j+1] + uh[j]);
      ...
      // calculate ph
      for (int j=0;j<core_x->n;j++)
	ph[j] = p[j]*exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * e(eff,k,t,d->Y) - kk)) - exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * e(eff,k,t,d->Y) - kk))*(k/2)*(1e-7*U+gg*P)*u[j];
