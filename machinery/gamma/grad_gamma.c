  ...
  */
  ...
  for (int i=1;i<=d.I;i++)
  {
	...
	Uh += .5 * (xh[j+1] - xh[j]) * (uh[j+1] + uh[j]);
      ...
      // calculate ph
      for (int j=0;j<=d.J;j++)
	ph[j] = p[j]*exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * _e(d.eff,t) - kk)) - exp(-(k/2)*(bb + gg*U + s(x[j]) * ii * _e(d.eff,t) - kk))*(k/2)*(1e-7*U+gg*P)*u[j];
      ...
      for (int j=d.J+1;j>0;j--)
	{
	  ...
	  pn[j] = p[j-1]*tmp1 - tmp1 * k*(1e-7*Uh+gg*Ph)*uh[j-1] * tmp2;
	}
