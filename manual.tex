...
\begin{equation}
J(\theta) = \int_0^T \int_0^\omega |s(x)w(x)f(t)u(x,t) - c(x,t)| \,dx\,dt
\end{equation}
The gradient of $J$ with respect to $\theta$ is given by
\begin{equation}
\int_0^T \int_0^\omega
...
\begin{subequations}
  \begin{align}
    J(\theta_{k+1}) &\leq J(\theta_k) + c_1 \zeta_k \nabla J(\theta_k)' \mathbf{q}_k \label{eq:w1}\\
    \nabla J(\theta_{k+1})' &\mathbf{q}_k \geq c_2 \nabla J(\theta_k)' \mathbf{q}_k\label{eq:w2}
  \end{align}
\end{subequations}
