...
\end{align}
\end{subequations}
where $x$ is the size of the individual, $\omega$ is the Memaximum size, and $b$, $g$ and $z$ represent the processes of birth, growth and death respectively.  Growth is 
\begin{equation}
...
\begin{equation}
  p(t) = \frac{\sum_{j=1}^m\phi_t((t-t_j)/h_t)}{\text{Memax}_t\sum_{j=1}^m\phi_t((t-t_j)/h_t)}.
\end{equation}
...
and $p(x,t)=u_\theta(x,t)$ are solutions of the the sensitivity-PDE %\citep{Borggaard1997}: 
\begin{equation}
