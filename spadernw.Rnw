...
However the McKendrick model has the same limitation as the Malthusian one - the mortality process is not dependent on the size of the population.  Clearly a Verhulstian effect is needed.  This was not achieved until \citet{Gurtin1974} proved existence and uniqueness of solutions to equations of the form
\begin{equation}
  \begin{split}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
which they showed reduces to Verhulst's model (with $\alpha_0 \equiv b_0 - b_1 - \mu_0$ and $\beta_0 \equiv \mu_1$) in the special case of a population that has an initial age distribution which is balanced in a certain sense. Away from this balance the initial birth rate is either `small' relative to the initial population, and the population initially decreases; alternatively it is `large', and the population surges to a maximum which is larger than its ultimate equilibrium value. 

While this class of models is impressively realistic, it still treats only age, which is often less appropriate than other physiological attributes \citep{Frank1960}. \citet{vanSickle1977} analysed a size-based version of the McKendrick PDE in the context of a fishery model and made the deep observation that a bump may appear in the size-frequency distribution purely due to fish growing into those size classes faster than they die or grow out of them (i.e. not because of an earlier recruitment pulse), but it wasn't until \citet{Murphy1983} that both density dependence and size were incorporated in the McKendrick PDE framework. \citet{Murphy1983}'s model is
\begin{equation}
  \begin{split}
...
%Interestingly \footnote{\citet{Sinko1967} give a rigorous derivation of an age \emph{and} size structured McKendrick PDE but their formulation lacks the critical $N$ term in the death function - density dependence is not taken into account} explain its relationship to Verhulst's model by integrating equation \ref{eq:1} over age and then setting $\mu$ to .. but they do not actually write down the PDE with population dependence inside it, nor do they seem to realise such a model is a different class from \ref{eq:1} and that the existence and uniqueness proof given in \citet{Sinko1968}.
...
\begin{equation}
  w(x)=w_1 x^{w_2}.
\end{equation}
This completes the specification of a simple fishery system.  The biological component of this system is a slight variant of that analysed by \citet{Murphy1983} - simpler in that growth and births are not density dependent, but more complex in that growth is size-dependent.  Another way to think of it is as the simplest modification of the first model in \citet{Gurtin1978} needed to handle size-dependent growth.  The extent to which this is a non-standard fishery model is taken up in section \ref{sec:disc}.
...
Equation \ref{eq:1} is a nonlinear hyperbolic partial differential equation with non-local terms and a nonlinear and non-local boundary condition.  Even without the time-dependent term (that is, even considering autonomous biological models of the kind described by Murphy) no analytical solutions are known. With it, a numerical approach is obviously required.  Significant research has been done in this area, for pointers into the literature see \citet{Angulo2004} and \citet{Angulo2014}. In this section we follow closely the scheme developed in \citet{Angulo2014}.

We start by reducing the PDE to a coupled ODE problem by integrating along the characteristic curves \citep{Courant1966} (this is analogous to following the mortality process of a particular cohort). Following \citet{Angulo2004}, define
\begin{equation}
...
\end{equation}
The coupled problems \ref{eq:2.2} and \ref{eq:2.5} are then simultaneously integrated together with boundary condition \ref{eq:1.1}.  This procedure is explained in detail in .. and .. however because it is so fundamental (and unfamiliar to fishery scientists) and connects to our later work, we include it here.. 
...
Substituting in the birth, growth and death functions, we get the following sensitivity PDEs see Appendix \ref{app:pde}

These PDEs can be reduced to coupled ODE problems along characteristics in a manner analogous to the original PDE case. 

For all derivatives the size ODE (Equation \ref{eq:2.2}) remains unchanged. For $\iota$ the `population' ODE is
\begin{equation}
  \begin{split}
...
This is implemented with Algorithm \ref{alg:sens}, this is one of the two main technical contributions of this paper. The other being the analytical gradients.
...
load('../data/bar2.RData')
barra <- subset(bar2, CAABSpeciesCommonName=='Barramundi')

bar2$Time <- (julian(bar2$StartDate) - julian(ymd("1990-01-01")))/365.25
...
cat <- array(0,dim=N+1)
eff <- array(0,dim=2*N+1)

karumba <- droplevels(subset(barra,GridDerived %in% c('AI16','AH16','AG17','AG18','AF17','AF18','AE17','AE18','AD17','AD18','AC18','AC17','AC16','AC15','AB15')))
karumba <- subset(karumba,StartDateYear >= StartYear)
karumba <- subset(karumba,StartDateYear <= EndYear)

for (i in 1:nrow(karumba)) {
    idxr <- floor((karumba$Time[i]+cek/2)/cek)
    idxr2 <- floor((karumba$Time[i]+k/2)/k)
    eff[idxr] <- eff[idxr] + 1.0/cek
    cat[idxr2] <- cat[idxr2] + (karumba$RetainedWholeWeightDerived[i]/k)/1e3
}
...
x <- matrix(0,nrow=I+1,ncol=J+I+2)
n <- matrix(0,nrow=I+1,ncol=J+I+2)
xh <- matrix(0,nrow=I+1,ncol=J+I+2)
nh <- matrix(0,nrow=I+1,ncol=J+I+2)
spb <- matrix(0,nrow=I+1,ncol=J+I+2)
SpB <- array(0,dim=I+1)    
...
zeta <- sqrt( 81*k*k*w*w*(a*A1+2*a*A2*w)^2 - 12*k*(a*A1*w+k)^3 )
eta <- 9*a*A1*k*k*w + 18*a*A2*k*k*w*w + k*zeta
Z <- eta^(1/3) / (3*(2/3)^(1/3)) + (2/3)^(1/3)*k*(a*A1*w+k) / eta^(1/3)
...
for (i in 2:(I+1)) {
    ...
    nh[i,1] <- Q2(xh[i,],nh[i,],g(0),J+i+1)

    Qhalf <- Q(xh[i,],nh[i,],J+i+1)
    ...
    for (j in 2:(J+i+1))
      n[i,j] <- n[i-1,j-1]*exp(-k*zstar(xh[i,j],Qhalf,th))
    n[i,1] <- Q2(x[i,],n[i,],g(0),J+i+1)

    Qi[i] <- Q(x[i,],n[i,],J+i+1)
