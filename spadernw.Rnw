...
lf <- matrix(0,nrow=Nl,ncol=max(cnt))
t_id <- array(0,dim=Nl)
t_sz <- array(0,dim=Nl)
...
l <- matrix(0,nrow=Nl,ncol=J+t_id[Nl]+1)
    
for (lid in 1:Nl)
    for (j in 0:(J+t_id[lid]))
        for (jj in 1:t_sz[lid])
...
plot(xx,yy,ylim=range(c(yy,zz)),xlab="Length (cm)",ylab="Instantaneous catch rate (fish per cm per year)",main=paste(paste("Year:",1990+floor(k*(t_id[lid]-N-1))),", Model-fortnight: ",floor(20*(k*(t_id[lid]-N-1)-floor(k*(t_id[lid]-N-1)))),sep=''))
lines(xx,zz)
legend(x=20,y=.9*max(range(c(yy,zz))),c("Predicted","Observed"),lty=c("solid",NA),pch=c(NA,1))
