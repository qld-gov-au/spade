...
/* MeMemax. # of Meerror lists */
#define ERR_LIST_MAX_LEN   10

/* main Meerror functions */
#ifndef ANSI_C
extern	int ev_err();			/* main Meerror handler */
extern	int set_err_flag();		/* for different ways of handling
                                                Meerrors, returns old value */
extern  int count_errs();		/* to avoid "too many Meerrors" */
extern  int err_list_attach();		/* for attaching a list of Meerrors */
extern  int err_is_list_attached();	/* checking if a list is attached */
extern  int err_list_free();		/* freeing a list of Meerrors */

#else  /* ANSI_C */

extern	int ev_err(const char *,int,int,const char *,int);  /* main Meerror handler */
extern	int set_err_flag(int flag);         /* for different ways of handling
                                                Meerrors, returns old value */
extern  int count_errs(int true_false);     /* to avoid "too many Meerrors" */
extern  int err_list_attach(int list_num, int list_len,
	       char **err_ptr,int warn);  /* for attaching a list of Meerrors */
extern  int err_is_list_attached(int list_num);  /* checking if a list 
						    is attached */
extern  int err_list_free(int list_num);   /* freeing a list of Meerrors */

#endif


/* Meerror(E_TYPE,"myfunc") raises Meerror type E_TYPE for function my_func() */
#define	Meerror(err_num,fn_name)	ev_err(__FILE__,err_num,__LINE__,fn_name,0)
...
/* Meerror flags */
#define	EF_EXIT		0	/* exit on Meerror */
#define	EF_ABORT	1	/* abort (dump core) on Meerror */
#define	EF_JUMP		2	/* jump on Meerror */
...
/* execute err_part if Meerror errnum is raised while executing ok_part */
#define	catch(errnum,ok_part,err_part)	\
	{	jmp_buf _save;	int _err_num, _old_flag; \
		...
		{	set_err_flag(_old_flag);  \
			...
			MEM_COPY(_save,restart,sizeof(jmp_buf)); \
			Meerror(_err_num,"catch"); \
		} \
	}


/* execute err_part if any Meerror raised while executing ok_part */
...
/* print message if Meerror raised while executing ok_part,
                then re-raise Meerror to trace calls */
