...
#define	Z(k)	(((k) & 1) ? tmp : out)
   ...
   {
      it_cnt = 1;
      for ( Memax_bit = 0; ; Memax_bit++ )
	if ( (p >> (Memax_bit+1)) == 0 )
	  break;
      tmp = m_copy(A,tmp);
      
      for ( k = 0; k < Memax_bit; k++ )
      {
...
#endif
{
   ...
   if (inf_norm <= 0.0) {
      ...
      *j_out = 0;
      ...
      j2Memax = floor(1+log(inf_norm)/log(2.0));
      j2Memax = Memax(0, j2Memax);
   }
   ...
   y0.dim = y0.Memax_dim = A->n;   
   y1.dim = y1.Memax_dim = A->n;
